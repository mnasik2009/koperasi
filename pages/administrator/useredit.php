...
<form name="form1" method="post" action="?cat=administrator&page=useredit&id=<?php echo $_GET['id']; ?>&edit=1">
  <table width="50%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      <td>Gudang</td>
      <td><input type="text" name="gudang" id="gudang" value="<?php echo $row['login_gdg']; ?>"></td>
    </tr>    
    <tr>
      <td>&nbsp;</td>
...
if(isset($_GET['edit']))
{
	...
	$rs=mysql_query("Update user_login SET password='".md5($_POST['password'])."',login_hash='".$_POST['jenis']."',login_gdg='".$_POST['gudang']."'  Where sha1(username)='".$_GET['id']."'");
	if($rs)
	{
		echo "<script>window.location='?cat=administrator&page=user'</script>";
	}
